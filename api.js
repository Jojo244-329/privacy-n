...
app.post('/api/gerar-pix', async (req, res) => {
  try {
    ...
    items: [
      ...
      const token = "sk_live_eZHaPhLd4xDo72Hh5KnruZUwDzfDkLfnsCkQBuF5TZS0RgQb";
      const auth = Buffer.from(`${token}:x`).toString("base64");
  
      const response = await axios.post("https://api.allowpay.online/functions/v1/transactions", payload, {
        headers: {
